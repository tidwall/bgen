...
C API for the [btree.h](https://github.com/tidwall/btree.h) B-tree generator.

This document provides a description of the functions and types in the btree.h source file.

It's recommended to first read the more general overview in the project [README](https://github.com/tidwall/btree.h).

Below is a complete list of all function generated by btree.h. 
The "bt" namespace and "bitem" item type are used to as a placeholder.
These are both configurable by the developer using BTREE_NAME and BTREE_TYPE.

```c
#define BTREE_NAME bt
#define BTREE_TYPE bitem
```
...
/// Returns bt_UNSUPPORTED when BTREE_NOORDER
...
The following operations are available when BTREE_COUNTED is provided to the 
generator. See [Counted B-tree](#counted-b-tree) for more information. Also,
when BTREE_NOORDER is provided, the btree effectively becomes a 
[Vector B-tree](#vector-b-tree).

```c
/// Insert an item at index
///
/// Unless BTREE_NOORDER is begin used, it's an error to attempt to insert an
...
///
/// Unless BTREE_NOORDER is begin used, it's an error to attempt to replace an
...
The following operations are available when BTREE_COW is used.
